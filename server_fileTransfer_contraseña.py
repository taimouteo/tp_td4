...
def generar_html_interfaz(modo):
    ...
    - Si modo == 'download': pide contraseña y luego hace GET /download?pass=[contraseña ingresada]
    - Si modo == 'upload': pide contraseña junto con el archivo.
    """
    if modo == 'download':
        return """
<html>
  <head>
    ...
    <style>
      ...
      input[type="password"], input[type="submit"] { padding: 10px; }
      input[type="submit"] { background: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer; }
    </style>
  </head>
  <body>
...
<html>
  <head>
    ...
    <style>
      body { font-family: sans-serif; max-width: 500px; margin: 50px auto; }
      form { display: flex; flex-direction: column; gap: 10px; padding: 20px; border: 2px dashed #ccc; border-radius: 5px; }
      input[type="file"], input[type="password"], input[type="submit"] { padding: 10px; }
      input[type="submit"] { background: #28a745; color: white; border: none; border-radius: 5px; cursor: pointer; }
    </style>
  </head>
  <body>
    <h1>Subir archivo</h1>
    <form method="POST" enctype="multipart/form-data" 
            onsubmit="this.action='/upload?pass=' + encodeURIComponent(this.pass.value)">
        <input type="password" name="pass" placeholder="Contraseña" required>
...
def start_server(archivo_descarga=None, modo_upload=False, usar_gzip=False):
    ...
    while True:
        ...
        if method == "GET":
            ...
            #   ii. Descarga
            elif not modo_upload and path.startswith("/download"):
                # CONTRASEÑA: Parseamos el path
                parsed = urlparse(path)  
                params = parse_qs(parsed.query)

                pass_get = params.get("pass", [None])[0] # Obtenemos la contraseña

                # Comparamos con la guardada
                if pass_get != contraseña:
                    body = b"Acceso denegado: contrasena incorrecta"
                    headers = (
